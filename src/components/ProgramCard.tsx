...
const ProgramCard: React.FC<ProgramCardProps> = ({ icon: Icon, title, description, image, slug }) => {
  return (
    <Link to={`/program/${slug}`} className="block"> {/* Wrap with Link */}
      <div className="bg-surface rounded-none transition-all duration-300 overflow-hidden group border border-transparent hover:border-accent">
        ...
        <div className="p-8">
          ...
          <span className="text-accent font-semibold hover:text-accent-hover transition-colors"> {/* Change a to span */}
            Läs mer →
          </span>
        </div>
      </div>
